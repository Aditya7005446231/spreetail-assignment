...
// Simple in-memory cache for historical USD to INR rates
const rateCache = {};
...
export async function fetchExchangeRate(date) {
  ...
  // Ensure date is in proper YYYY-MM-DD format, fallback to standard date if not
  ...
  try {
    // Timeout of 3 seconds to avoid blocking imports indefinitely
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 3000);

    const res = await fetch(`https://api.frankfurter.app/${formattedDate}?from=USD&to=INR`, {
    ...
    if (res.ok) {
      ...
      if (data.rates && data.rates.INR) {
        ...
        rateCache[date] = rate;
        console.log(`Fetched historical USD/INR rate for ${formattedDate}: ${rate}`);
        return rate;
      }
    }
  } catch (err) {
    console.error(`Error fetching historical rate for ${formattedDate}:`, err.message);
  }

  // Fallback rate if API fails or times out
  return 83.0;
}

export async function parseAndValidateCSV(csvContent) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!curr) {
      ...
      const liveRate = await fetchExchangeRate(dateVal);
      convertedAmountInr = amountVal * liveRate;
      anomalies_detected.push({
        type: "currency_usd",
        desc: `Currency is in USD ($ ${amountVal}). Auto-converting to INR using historical exchange rate for ${dateVal}: 1 USD = ${liveRate} INR`,
        suggested: `Convert to Rs. ${Number(convertedAmountInr).toFixed(2)} (1 USD = ${liveRate} INR)`
      });
    }
